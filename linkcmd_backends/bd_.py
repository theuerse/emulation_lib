...
class BD_(backend.Backend):
    ...
    def get_indiv_setup_commands(self, config, mapping, line, local_id, remote_id):
        ...
        # limit = " limit " + str(int((burst * 8) / target_bitrate)) if target_bitrate > 0 else ""
        limit = " limit " + str(10)
        local_setup_cmds.append("sudo tc qdisc add dev " + config["EMU_INTERFACE"] + " parent " + remote_hostByte +
                                ": handle " + str(10 * int(remote_hostByte)) + ": netem delay " + str(target_delay) + "ms" + limit)
        # TODO: set limit for delay in order to avoid super-large delays: https://www.duo.uio.no/bitstream/handle/10852/37459/Moe-Master.pdf
